...
#line="/lab/solexa_yamashita/people/nilu/Nilu_s_UROP_at_Yamashita_Lab/ncbi_data_directory_0.1/top_0/main_1/Plasmodium_coatneyi_genome/ncbi_dataset/data/GCF_001680005.1/genomic.gtf,Plasmodium_coatneyi_genome"
        try:
            
            gtf_loc, name = line.split(",")
...
for genomeId, meta_dico in genome_Metadata.items():
    ...
    ax.scatter((size*0.001), (max_intron*0.001), color = style['color'])
    ax.set_title("genome size vs max intron (scale = kbp)")
    ax.legend()
    ax1.scatter(math.log10(size*0.001), math.log10(max_intron*0.001), color = style['color'] )
    ax1.set_title("genome size vs max intron (scale = log bp)")
    ax1.legend()

# plt.show()  
logger.info(_mapping)   
    
fig2, ax2 = plt.subplots(1)
logger.info(_kingdom_count)
ax2.pie(_kingdom_count.values(), labels = _kingdom_count.keys())

fig3, ax3 = plt.subplots(1)
ax3.pie(_type_count.values(), labels = _type_count.keys())
logger.info(_type_count)

fig4, ax4 = plt.subplots(1)
ax4.pie(_status_count.values(), labels = _status_count.keys())
logger.info(_status_count)
